...
int main(int argc, char *argv[])
{
	...
	mk_it("Gold ring", "A gold ring that glistens with a bright shine");
	mk_it("Diamond ring", "A diamond ring that sparkles almost hurts your eyes");
	mk_it("Parchment scrap", "A scrap of parchment");
	mk_it("Scroll", "A scroll that is rolled up and bound with string of leather");

	mk_npc("Charlotte", "A very sweet lady spider",
				 (std::unordered_map<std::string, std::tuple<std::string, textworld::core::action_func>>{
						 {"hello", std::make_tuple("Have you seen Wilbur? I've been looking around everywhere for him...", nullptr)}}));

	mk_npc("Old Man", "A really old man",
				 (std::unordered_map<std::string, std::tuple<std::string, textworld::core::action_func>>{
						 {"hello", std::make_tuple("Hi there, take this silver ring!!!",
																			 [](std::shared_ptr<textworld::ecs::Entity> player_entity, std::shared_ptr<textworld::ecs::EntityManager> entity_manager)
																			 {
	...
	mk_rm("Hollow Tree", "You stepped into a large hollow tree. It's damp in here.");
	...
	mk_ex("Dark Passage", "Hollow Tree", textworld::data::Direction::NORTH);

	pl_it("Open Field", "Coin Purse", 1);
	pl_it("Open Field", "Health Potion", 3);
	pl_it("Large Rock", "Lamp", 1);
	pl_it("Hollow Tree", "Gold ring", 1);
	pl_it("Hollow Tree", "Scroll", 1);
	...
	auto player_entity = textworld::helpers::make_player(entity_manager,
																											 "player 1",
																											 entity_manager->get_entity_id_by_name("rooms", "Open Field"),
